...
class SimpleTrainer(TrainerBase):
    ...
    def after_epoch(self):
        ...
        elif do_test and self.cfg.TEST.FINAL_MODEL == "base_epoch":
            if self.epoch > 200 and self.epoch % 50 == 0:
                curr_result = self.test(split="val")
                self.save_model(
                    self.epoch,
                    self.output_dir,
                    val_result=curr_result,
                    model_name="epoch{}_acc_{}.pth.tar".format(self.epoch, round(curr_result, 2))
                )
